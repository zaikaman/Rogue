...
export default function APYCard({ title, value, change, protocol }: APYCardProps) {
  ...
  return (
    <div className="terminal-border bg-noir-dark/50 backdrop-blur rounded-sm p-6 scan-line hover:glow-red transition-all duration-300 group">
      <div className="flex items-start justify-between mb-4">
        <div className="font-mono text-xs text-gray-500 uppercase tracking-wider">
          {title}
        </div>
        <div className="px-2 py-1 bg-noir-gray/50 rounded text-xs font-mono text-red-glow">
          {protocol}
        </div>
      </div>
      ...
      <div className="mt-4 h-1 bg-noir-gray rounded-full overflow-hidden">
        <div className="h-full bg-gradient-red w-3/4 group-hover:w-full transition-all duration-500" />
      </div>
    </div>
  )
}
